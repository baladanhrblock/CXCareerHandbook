...
import { CORE_ROWS, PRACTICE_ROWS } from "../data/sharedCompetencies";
...
// ─── Craft preface: discipline description + skill chips ──────────────────────

function CraftPreface({ discipline }: { discipline: DisciplineData }) {
  return (
...
export function UnifiedHandbook({
  ...
  // Announce craft-section changes to assistive technology
  useEffect(() => {
    setAnnouncement(`Now showing ${data.title} craft competencies.`);
  }, [discipline, data.title]);
  ...
  sections.push({
    ...
    sticky: true,
    preface: <CraftPreface discipline={data} />,
